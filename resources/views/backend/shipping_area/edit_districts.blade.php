...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    <div class="breadcrumb-title pe-3">Edit Districts</div>
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                <li class="breadcrumb-item"><a href="javascript:;"><i class="bx bx-home-alt"></i></a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">Edit District Page</li>
            </ol>
        </nav>
    ...
    <div class="ms-auto">
        <div class="btn-group">
            <a href="{{route('all.districts')}}" class="btn btn-primary"> All District</a>
        </div>
	</div>
</div>
<div class="col-lg-12" bis_skin_checked="1">
    <div class="card" bis_skin_checked="1">
        <div class="card-body" bis_skin_checked="1">
            ...
            <form action="{{route('update.districts')}}" id ="myForm" method="post">
                @csrf
                   <input type="hidden" name="district_id" value="{{$dist->id}}">
                ...
                <select class="form-select" name ="division_id"  aria-label="Default select example">
                            ...
                            @foreach ( $divi as $div)
                            <option value="{{$div->id}}" {{$div->id == $dist->division_id ? 'selected' : '' }}> {{$div->division_name}}</option>
                            @endforeach
                        </select>      
                </div>
              </div>
                ...
                <div class="col-sm-9 text-secondary form-group" bis_skin_checked="1">
                    <input type="text" name="distrits_name" class="form-control" value="{{$dist->district_name}}">
